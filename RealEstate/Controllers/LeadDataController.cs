...
namespace RealEstate.Controllers
{
    public class LeadDataController : Controller
    {
        ...
        //        {
        //            var Total = 0;
        //            var reviews = DB.TblLeads.Where(x => x.AccountId == AccountId).ToList(); //.Include(x => x.Agent)
        //            if (Search != null)
        //            {
        ...
        public IActionResult GetAllLead()
        {
            ...
            int AccountId = Convert.ToInt32(Request.Cookies["LoginAccountId"]);
            var oLeadList = _dbContext.TblLeads.Where(x => x.AccountId == AccountId).ToList().Select(s => new LeadViewModel //.Include(x => x.Agent)
            {
                LeadId = s.LeadId,
